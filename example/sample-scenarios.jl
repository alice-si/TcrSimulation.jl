...
function differentMechanisms()
    ...
    for acc in 0:5:100
        ...
        token = mean([simToken(1000, setupAgentsWithFixedAccuracy(100, acc), noneRedistribution, [benchmarkRegistryMean]) for i in 1:20])[1]
...
function tokensAndDiversifications()
    ...
    for acc in 0:5:100
        ...
        tokenFixed = mean([simToken(1000, setupAgentsWithFixedAccuracy(100, acc), noneRedistribution, [benchmarkRegistryMean]) for i in 1:30])[1]
        tokenDiv = mean([simToken(1000, setupRandomAgents(100, acc, 20), noneRedistribution, [benchmarkRegistryMean]) for i in 1:30])[1]
...
function efficiencyArea()
    for noAgents in 10:10:200
        for noSteps in 100:100:2000
            ...
            token = [simToken(noSteps, setupRandomAgents(noAgents, 50, 20), noneRedistribution, [benchmarkRegistryMean])[1] for i in 1:10]
            boost = mean(token) - mean(noToken)
...
function expertsInnerCirleSize()
    for expertCount in 1:1:50
        ...
        expertTokens = mean([simToken(1000, setupMixedAgents(100-expertCount, 20, expertCount, 100), noneRedistribution, [benchmarkRegistryMean])[1] for i in 1:20])
