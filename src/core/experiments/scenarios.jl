function compareRegistryQuality(rng)
    for acc in 10:10:100
        noChallenge = mean([simFixedAgentsNoChallenge(rng, 1000, 50, acc, [benchmarkRegistryMean]) for i in 1:10])[1]
        challenge = mean([simFixedAgentsWithChallenge(rng, 1000, 50, acc, [benchmarkRegistryMean]) for i in 1:10])[1]
        divAgents = mean([simDiversifiedAgentsWithChallenge(rng, 1000, 50, acc, 20, [benchmarkRegistryMean]) for i in 1:10])[1]
        token = mean([simWithProRataTokens(rng, 1000, 50, acc, 20, [benchmarkRegistryMean]) for i in 1:10])[1]

        println("$acc, $noChallenge, $challenge, $divAgents, $token")
    end
end

function compareEfficiency(rng)
    for acc in 0:5:100
        noToken = [simDiversifiedAgentsWithChallenge(rng, 1500, 50, acc, 20, [benchmarkRegistryMean])[1] for i in 1:10]
        token = [simWithProRataTokens(rng, 1500, 50, acc, 20, [benchmarkRegistryMean])[1] for i in 1:10]
